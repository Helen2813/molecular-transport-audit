...
SCRIPT_VERSION = "05b-run-final-matched-mapping-specificity-null-gpu-v2-no-cli"
...
DIRECT_SUMMARY = (
    ...
    / "primary_pooled_direct_preservation_summary_v2.tsv"
)
...
OUT_DIR = DATA_ROOT / "paper4_tcbb_final_mapping_specificity_null_v2"
...
def load_scanb_target(
    ...
    if len(genes) != 9220:
        raise RuntimeError(f"Expected 9,220 corrected-evaluable SCAN-B genes, got {len(genes)}.")
...
def load_metabric_target(universe_set: set[str]) -> dict:
    ...
    if len(genes) != 8485:
        raise RuntimeError(f"Expected 8,485 corrected-evaluable METABRIC genes, got {len(genes)}.")
...
def program_paths(target: str, program_id: str) -> dict[str, Path]:
    ...
    return {
        "result": d / f"{program_id}_mapping_specificity_result_v2.json",
        "null": d / f"{program_id}_mapping_specificity_null_v2.npz",
        "quality": d / f"{program_id}_mapping_panel_quality_v2.tsv",
        "example": d / f"{program_id}_mapping_example_panel1_v2.tsv",
    }
...
def main() -> None:
    print("=" * 142)
    print("Paper 4 / TCBB - corrected 1,000-panel matched-mapping specificity null + primary classification")
    print("=" * 142)
    ...
    scanb_eval_symbols = {
        ...
        for x in scanb_eval.loc[
            scanb_eval["statistically_evaluable_corrected"].astype(str) == "1",
            "Hugo_Symbol",
        ]
    }
    met_eval_symbols = {
        ...
        for x in met_eval.loc[
            met_eval["statistically_evaluable_corrected"].astype(str) == "1",
            "Hugo_Symbol",
        ]
    }

    if len(scanb_eval_symbols) != 9220:
        raise RuntimeError(f"Expected 9,220 corrected-evaluable SCAN-B feature genes, got {len(scanb_eval_symbols)}.")
    ...
    summary_path = OUT_DIR / "primary_pooled_final_classification_v2.tsv"
    classification.to_csv(summary_path, sep="\t", index=False)

    spec_df = pd.DataFrame(all_spec_results)
    spec_path = OUT_DIR / "mapping_specificity_summary_v2.tsv"
    ...
    master = {
        "script_version": SCRIPT_VERSION,
        "status": "CORRECTED_FINAL_PRIMARY_POOLED_CLASSIFICATION_COMPLETE",
    ...
    master_path = OUT_DIR / "final_mapping_specificity_and_classification_v2.json"
    master_path.write_text(json.dumps(master, indent=2), encoding="utf-8")

    print("\n" + "=" * 142)
    print("05b v2 CORRECTED MATCHED-MAPPING SPECIFICITY NULL: COMPLETE")
    print("=" * 142)
