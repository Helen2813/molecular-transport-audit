...
SCRIPT_VERSION <- "05i-run-postprimary-comparator-benchmark-v1-no-cli"
...
for (target in names(TARGETS)) {
  ...
  netrep_rds <- file.path(
    target_dir,
    "netrep_modulePreservation_v1.rds"
  )
  netrep_tsv <- file.path(
    target_dir,
    "netrep_modulePreservation_long_v1.tsv"
  )
  ...
  wgcna_rds <- file.path(
    target_dir,
    "wgcna_modulePreservation_v1.rds"
  )
  wgcna_tsv <- file.path(
    target_dir,
    "wgcna_modulePreservation_summary_v1.tsv"
  )
  ...
  integrated_path <- file.path(
    target_dir,
    "integrated_mta_netrep_wgcna_comparison_v1.tsv"
  )
  write.table(
    integrated,
    integrated_path,
...
netrep_all_path <- file.path(
  OUT_DIR,
  "netrep_all_targets_long_v1.tsv"
)
wgcna_all_path <- file.path(
  OUT_DIR,
  "wgcna_modulePreservation_all_targets_v1.tsv"
)
integrated_all_path <- file.path(
  OUT_DIR,
  "mta_netrep_wgcna_integrated_all_targets_v1.tsv"
)
...
master_path <- file.path(
  OUT_DIR,
  "postprimary_comparator_benchmark_v1.json"
)
jsonlite::write_json(
  master,
  master_path,
