...
SCRIPT_VERSION <- "05i-run-postprimary-comparator-benchmark-v5-no-cli"
...
NETREP_THREADING_AMENDMENT_JSON <- file.path(
  DATA_ROOT,
...
load_scanb_target <- function(common_genes) {
  ...
  cat("  SCAN-B identifier candidates with positive overlap:\n")
  positive_idx <- which(candidate_unique_overlap > 0)
  ...
  cat(
    "  zero-overlap identifier candidates: ",
    sum(candidate_unique_overlap == 0),
    "\n",
    sep = ""
  )
...
cat("  05i5 NetRep threading amendment frozen:            8 threads\n")
cat(strrep("=", 152), "\n\n", sep = "")
...
required <- c(
  ...
  NETREP_THREADING_AMENDMENT_JSON,
  SOURCE_MATRIX,
  SOURCE_MEMBERSHIP,
  SCANB_EXPR,
  SCANB_PRIMARY,
...
thread_amendment <- jsonlite::read_json(
  NETREP_THREADING_AMENDMENT_JSON,
  simplifyVector = TRUE
)
if (as.character(thread_amendment$status) !=
    "FROZEN_BEFORE_FIRST_COMPARATOR_STATISTIC") {
  stop("05i5 NetRep threading amendment has unexpected status.")
}
if (as.integer(thread_amendment$NetRep$amended_nThreads) != NETREP_THREADS) {
  stop("05i5 amendment does not match runner NetRep nThreads.")
}
...
for (target in names(TARGETS)) {
  ...
  netrep_rds <- file.path(
    target_dir,
    "netrep_modulePreservation_v5.rds"
  )
  netrep_tsv <- file.path(
    target_dir,
    "netrep_modulePreservation_long_v5.tsv"
  )
  ...
  wgcna_rds <- file.path(
    target_dir,
    "wgcna_modulePreservation_v5.rds"
  )
  wgcna_tsv <- file.path(
    target_dir,
    "wgcna_modulePreservation_summary_v5.tsv"
  )
  ...
  integrated_path <- file.path(
    target_dir,
    "integrated_mta_netrep_wgcna_comparison_v5.tsv"
  )
  write.table(
    integrated,
    integrated_path,
...
netrep_all_path <- file.path(
  OUT_DIR,
  "netrep_all_targets_long_v5.tsv"
)
wgcna_all_path <- file.path(
  OUT_DIR,
  "wgcna_modulePreservation_all_targets_v5.tsv"
)
integrated_all_path <- file.path(
  OUT_DIR,
  "mta_netrep_wgcna_integrated_all_targets_v5.tsv"
)
...
master <- list(
  ...
  netrep_threading_amendment = NETREP_THREADING_AMENDMENT_JSON,
...
master_path <- file.path(
  OUT_DIR,
  "postprimary_comparator_benchmark_v5.json"
)
jsonlite::write_json(
  master,
  master_path,
