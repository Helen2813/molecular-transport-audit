...
SCRIPT_VERSION = "05c-run-primary-bootstrap-reliability-gpu-v1-no-cli"
...
OUT_DIR = DATA_ROOT / "paper4_tcbb_primary_bootstrap_reliability_v1"
...
def checkpoint_paths(target: str, program_id: str) -> dict[str, Path]:
    ...
    return {
        "checkpoint": d / f"{program_id}_bootstrap_checkpoint_v1.npz",
        "final_npz": d / f"{program_id}_bootstrap_reliability_arrays_v1.npz",
        "result": d / f"{program_id}_bootstrap_reliability_result_v1.json",
    }
...
def bootstrap_program(
    ...
    if paths["checkpoint"].exists():
        ...
        if (
            ...
            print(f"    resuming bootstrap from {completed}/{BOOTSTRAP_REPLICATES}")

    rng = np.random.default_rng(bootstrap_seed)

    # Replay RNG state deterministically up to completed bootstrap.
    ...
    source_edge_rank_norm_t = torch.tensor(
        source_edge_rank_norm,
        dtype=torch.float64,
        device=device,
    )
    source_loading_rank_norm_t = torch.tensor(
        source_loading_rank_norm,
        dtype=torch.float64,
        device=device,
    )
    ...
    for b in range(completed, BOOTSTRAP_REPLICATES):
        idx_np = rng.integers(
            0,
            target_x.shape[0],
            size=target_x.shape[0],
        ...
        if torch.any(~torch.isfinite(sd)) or torch.any(sd <= 0):
            raise RuntimeError(
                f"{target_name} {program_id}: zero/nonfinite SD in bootstrap {b+1}"
            )
        zb = (xb - mu) / sd
        ...
        del idx_t, xb, mu, sd, zb, corr, edges_t, vb_t

        if (
            (b + 1) % CHECKPOINT_EVERY == 0
            or (b + 1) == BOOTSTRAP_REPLICATES
        ):
            np.savez_compressed(
                paths["checkpoint"],
            ...
            print(
                f"    bootstraps {b+1:4d}/{BOOTSTRAP_REPLICATES}: "
    ...
    del (
        ...
        source_edge_rank_norm_t,
        source_loading_rank_norm_t,
        init_t,
        sign_t,
    )
    torch.cuda.empty_cache()
...
def main() -> None:
    ...
    print(f"  Target-sample bootstraps per assessable combination: {BOOTSTRAP_REPLICATES:,}")
    ...
    summary_tsv = OUT_DIR / "primary_pooled_bootstrap_reliability_summary_v1.tsv"
    out_df.to_csv(
        summary_tsv,
        sep="\t",
        index=False,
    ...
    master = {
        "script_version": SCRIPT_VERSION,
        "status": "PRIMARY_POOLED_BOOTSTRAP_RELIABILITY_COMPLETE",
        "bootstrap_replicates": BOOTSTRAP_REPLICATES,
    ...
    master_json = OUT_DIR / "primary_pooled_bootstrap_reliability_v1.json"
    ...
    print("\n" + "=" * 142)
    print("05c PRIMARY POOLED BOOTSTRAP / RELIABILITY: COMPLETE")
    print("=" * 142)
