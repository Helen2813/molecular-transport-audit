...
SCRIPT_VERSION = "04i-freeze-target-statistical-evaluability-amendment-v2-no-cli"
...
OUT_DIR = DATA_ROOT / "paper4_tcbb_target_statistical_evaluability_amendment_v2"
...
def main() -> None:
    ...
    print("  v1 execution status: failed before target evaluability was calculated (missing function argument only).")
    print()
    ...
    if "include_in_primary_scanb_analysis" not in primary.columns:
        raise RuntimeError("SCAN-B primary manifest lacks include_in_primary_scanb_analysis.")
    if not (primary["include_in_primary_scanb_analysis"].astype(str) == "1").all():
        raise RuntimeError("SCAN-B primary manifest contains a row not frozen for primary analysis.")
    ...
    if len(set(primary_titles)) != len(primary_titles):
        raise RuntimeError("SCAN-B frozen primary titles are not unique.")

    print("\n[1/3] SCAN-B statistical evaluability ...")
    scanb = load_scanb_target_stats(universe, primary_titles)
    ...
    contract = {
        "contract_id": "paper4-tcbb-target-statistical-evaluability-amendment-v2",
    ...
    scanb.to_csv(
        OUT_DIR / "scanb_target_statistical_evaluability_v2.tsv",
        sep="\t",
        index=False,
    )
    met.to_csv(
        OUT_DIR / "metabric_target_statistical_evaluability_v2.tsv",
        sep="\t",
        index=False,
    )
    affected.to_csv(
        OUT_DIR / "mapped_but_statistically_ineligible_genes_v2.tsv",
        sep="\t",
        index=False,
    )
    impact.to_csv(
        OUT_DIR / "program_assessability_after_statistical_evaluability_v2.tsv",
        sep="\t",
        index=False,
    )

    json_out = OUT_DIR / "target_statistical_evaluability_amendment_v2.json"
    json_out.write_text(json.dumps(contract, indent=2), encoding="utf-8")

    print("\n" + "=" * 136)
    print("04i v2 TARGET STATISTICAL-EVALUABILITY AMENDMENT: PASS")
    print("=" * 136)
