...
SCRIPT_VERSION <- "05i-run-postprimary-comparator-benchmark-v6-no-cli"
...
V4_SCANB_NETREP_DIR <- file.path(
  DATA_ROOT,
  "paper4_tcbb_postprimary_comparator_benchmark_v6",
  "SCANB_GSE96058"
)
V4_SCANB_NETREP_RDS <- file.path(
  V4_SCANB_NETREP_DIR,
  "netrep_modulePreservation_v6.rds"
)
V4_SCANB_NETREP_TSV <- file.path(
  V4_SCANB_NETREP_DIR,
  "netrep_modulePreservation_long_v6.tsv"
)
...
for (target in names(TARGETS)) {
  ...
  if (target == "SCANB_GSE96058") {
    ...
    if (!file.copy(V4_SCANB_NETREP_RDS, netrep_rds, overwrite = TRUE)) {
      stop("Failed to copy authoritative SCAN-B NetRep RDS into v6 output.")
    }
    if (!file.copy(V4_SCANB_NETREP_TSV, netrep_tsv, overwrite = TRUE)) {
      stop("Failed to copy authoritative SCAN-B NetRep TSV into v6 output.")
    }
  ...
  wgcna_rds <- file.path(
    target_dir,
    "wgcna_modulePreservation_v6.rds"
  )
  wgcna_tsv <- file.path(
    target_dir,
    "wgcna_modulePreservation_summary_v6.tsv"
  )
  ...
  integrated_path <- file.path(
    target_dir,
    "integrated_mta_netrep_wgcna_comparison_v6.tsv"
  )
  write.table(
    integrated,
    integrated_path,
...
netrep_all_path <- file.path(
  OUT_DIR,
  "netrep_all_targets_long_v6.tsv"
)
wgcna_all_path <- file.path(
  OUT_DIR,
  "wgcna_modulePreservation_all_targets_v6.tsv"
)
integrated_all_path <- file.path(
  OUT_DIR,
  "mta_netrep_wgcna_integrated_all_targets_v6.tsv"
)
...
master_path <- file.path(
  OUT_DIR,
  "postprimary_comparator_benchmark_v6.json"
)
jsonlite::write_json(
  master,
  master_path,
