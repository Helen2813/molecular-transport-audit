...
SCRIPT_VERSION = "05a-run-primary-pooled-direct-preservation-gpu-v1-no-cli"
...
OUT_DIR = DATA_ROOT / "paper4_tcbb_primary_pooled_direct_preservation_v1"
...
def load_scanb_target(
    ...
    if len(genes) != 9225:
        raise RuntimeError(f"Expected 9,225 statistically evaluable SCAN-B genes; found {len(genes)}.")
...
def load_metabric_target(universe_set: set[str]) -> dict:
    ...
    if len(genes) != 8485:
        raise RuntimeError(f"Expected 8,485 statistically evaluable METABRIC genes; found {len(genes)}.")
...
def program_result_paths(target_name: str, program_id: str) -> tuple[Path, Path, Path]:
    ...
    return (
        d / f"{program_id}_direct_result_v1.json",
        d / f"{program_id}_direct_nulls_v1.npz",
        d / f"{program_id}_evaluable_genes_v1.tsv",
    )
...
def main() -> None:
    print("=" * 140)
    print("Paper 4 / TCBB - FIRST TRUE TARGET ANALYSIS: pooled direct edge/loading preservation with frozen inference")
    print("=" * 140)
    ...
    print("  Target gene evaluability frozen before preservation: YES")
    ...
    summary_tsv = OUT_DIR / "primary_pooled_direct_preservation_summary_v1.tsv"
    ...
    master = {
        "script_version": SCRIPT_VERSION,
        "status": "DIRECT_POOLED_PRESERVATION_COMPLETE_MAPPING_NULL_PENDING",
    ...
    master_json = OUT_DIR / "primary_pooled_direct_preservation_v1.json"
    master_json.write_text(json.dumps(master, indent=2), encoding="utf-8")

    print("\n" + "=" * 140)
    print("05a PRIMARY POOLED DIRECT PRESERVATION: COMPLETE")
    print("=" * 140)
