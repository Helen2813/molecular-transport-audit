...
SCRIPT_VERSION <- "05i-run-postprimary-comparator-benchmark-v7-no-cli"
...
cat("  v4 SCAN-B parser retained:                         positional + row-name identifier audit\n")
cat("  05i3 interpretation matrix/head-to-head frozen:    YES\n")
cat("  05i4 NetRep permutation amendment frozen:          10,000 permutations\n")
cat("  05i5a continuation frozen:                          reuse SCAN-B NetRep v4 bytes\n")
cat("  Remaining NetRep runs use:                         8 threads\n")
cat("  v7 path-wiring bugfix:                             authoritative input=v4; fresh output=v7\n")
cat(strrep("=", 152), "\n\n", sep = "")
...
if (!requireNamespace("digest", quietly = TRUE)) {
  stop("R package 'digest' is required.")
}
...
partial_continuation <- jsonlite::read_json(
  PARTIAL_CONTINUATION_JSON,
  simplifyVector = TRUE
)
if (as.character(partial_continuation$status) !=
    "FROZEN_AFTER_SCANB_NETREP_BYTES_EXIST_BEFORE_NUMERICAL_INSPECTION") {
  stop("05i5a partial-continuation contract has unexpected status.")
}

current_rds_sha <- digest::digest(
  file = V4_SCANB_NETREP_RDS,
  algo = "sha256",
  serialize = FALSE
)
current_tsv_sha <- digest::digest(
  file = V4_SCANB_NETREP_TSV,
  algo = "sha256",
  serialize = FALSE
)
...
if (!identical(
      current_rds_sha,
      as.character(partial_continuation$authoritative_completed_artifact$rds$sha256)
    )) {
  stop("Authoritative SCAN-B NetRep v4 RDS hash changed.")
}
if (!identical(
      current_tsv_sha,
      as.character(partial_continuation$authoritative_completed_artifact$tsv$sha256)
    )) {
  stop("Authoritative SCAN-B NetRep v4 TSV hash changed.")
}
...
for (target in names(TARGETS)) {
  ...
  netrep_rds <- file.path(
    target_dir,
    "netrep_modulePreservation_v7.rds"
  )
  netrep_tsv <- file.path(
    target_dir,
    "netrep_modulePreservation_long_v7.tsv"
  )

  if (target == "SCANB_GSE96058") {
    cat("\n[NetRep] Reusing authoritative completed SCAN-B NetRep v4 bytes (hash-verified) ...\n")
    if (!file.copy(V4_SCANB_NETREP_RDS, netrep_rds, overwrite = TRUE)) {
      stop("Failed to copy authoritative SCAN-B NetRep RDS into v7 output.")
    }
    if (!file.copy(V4_SCANB_NETREP_TSV, netrep_tsv, overwrite = TRUE)) {
      stop("Failed to copy authoritative SCAN-B NetRep TSV into v7 output.")
    }

    netrep_long <- read.delim(
      netrep_tsv,
      sep = "\t",
      header = TRUE,
      check.names = FALSE,
  ...
  } else if (!file.exists(netrep_rds)) {
    ...
    cat(
      "[NetRep] Running ",
      NETREP_PERM,
      " permutations on all 12 frozen source modules using ",
      NETREP_THREADS_REMAINING,
      " threads ...\n",
      sep = ""
    )
    ...
    netrep_res <- NetRep::modulePreservation(
      ...
      nThreads = NETREP_THREADS_REMAINING,
      nPerm = NETREP_PERM,
      null = "overlap",
      alternative = "greater",
      simplify = FALSE,
  ...
  wgcna_tsv <- file.path(
    target_dir,
    "wgcna_modulePreservation_summary_v7.tsv"
  )
  ...
  integrated_path <- file.path(
    target_dir,
    "integrated_mta_netrep_wgcna_comparison_v7.tsv"
  )
  write.table(
    integrated,
    integrated_path,
...
wgcna_all_path <- file.path(
  OUT_DIR,
  "wgcna_modulePreservation_all_targets_v7.tsv"
)
integrated_all_path <- file.path(
  OUT_DIR,
  "mta_netrep_wgcna_integrated_all_targets_v7.tsv"
)
...
master <- list(
  ...
  partial_continuation_contract = PARTIAL_CONTINUATION_JSON,
  authoritative_scanb_netrep =
    "exact hash-frozen 05i v4 result, not recomputed",
  remaining_netrep_nThreads = NETREP_THREADS_REMAINING,
  ...
  continuation_runner_bugfix = paste(
    "v6 failed before data loading because authoritative-v4 paths were",
    "accidentally rewired to nonexistent v6 files. v7 corrects paths only;",
